...
namespace Application.TaskItems.Abstractions
{
    public interface ITaskItemWriteService
    {
        ...
        Task<DomainMutation> EditAsync(Guid taskId, Guid userId, string? newTitle, string? newDescription,
            DateTimeOffset? newDueDate, byte[] rowVersion, CancellationToken ct = default);

        Task<DomainMutation> MoveAsync(Guid taskId, Guid targetColumnId, Guid targetLaneId, Guid userId,
