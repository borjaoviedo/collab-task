...
namespace Application.TaskNotes.Realtime
{
    ...
    public sealed record TaskNoteCreatedEvent(Guid ProjectId, TaskNoteCreatedPayload Payload)
        : Application.Realtime.BoardEvent<TaskNoteCreatedPayload>("note.created", ProjectId, DateTimeOffset.UtcNow, Payload);

    public sealed record TaskNoteUpdatedEvent(Guid ProjectId, TaskNoteUpdatedPayload Payload)
        : Application.Realtime.BoardEvent<TaskNoteUpdatedPayload>("note.updated", ProjectId, DateTimeOffset.UtcNow, Payload);

    public sealed record TaskNoteDeletedEvent(Guid ProjectId, TaskNoteDeletedPayload Payload)
        : Application.Realtime.BoardEvent<TaskNoteDeletedPayload>("note.deleted", ProjectId, DateTimeOffset.UtcNow, Payload);
}
