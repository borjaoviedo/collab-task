...
namespace Application.TaskAssignments.Realtime
{
    ...
    public sealed record TaskAssignmentCreatedEvent(Guid ProjectId, TaskAssignmentCreatedPayload Payload)
        : Application.Realtime.RealtimeEvent<TaskAssignmentCreatedPayload>("assignment.created", ProjectId, DateTimeOffset.UtcNow, Payload);

    public sealed record TaskAssignmentUpdatedEvent(Guid ProjectId, TaskAssignmentUpdatedPayload Payload)
        : Application.Realtime.RealtimeEvent<TaskAssignmentUpdatedPayload>("assignment.updated", ProjectId, DateTimeOffset.UtcNow, Payload);

    public sealed record TaskAssignmentRemovedEvent(Guid ProjectId, TaskAssignmentRemovedPayload Payload)
        : Application.Realtime.RealtimeEvent<TaskAssignmentRemovedPayload>("assignment.removed", ProjectId, DateTimeOffset.UtcNow, Payload);
}
